...
@section('contenido')
    ...
    <div
        ...
        <div style="display:flex;align-items:center;gap:.75rem;">
            <a href="{{ route('recetas.index') }}"
                style="color:var(--texto-secundario);text-decoration:none;font-size:.84rem;">
    ...
    @if (session('exito'))
        <div
            style="background:#dcfce7;border:1px solid #86efac;color:#166534;border-radius:8px;padding:.7rem 1rem;margin-bottom:1rem;font-size:.84rem;">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-8">
            <div class="card-sistema" style="margin-bottom:1rem;">
                <div
                    style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:1rem;margin-bottom:1rem;padding-bottom:1rem;border-bottom:1px solid var(--fondo-borde);">
                    <div>
                        <div
                            style="font-size:.72rem;color:var(--texto-secundario);text-transform:uppercase;letter-spacing:.04em;">
                            Paciente</div>
                        <div style="font-weight:600;font-size:.9rem;">
                            <a href="{{ route('pacientes.show', $receta->paciente) }}"
                                style="color:var(--color-principal);text-decoration:none;">
                                {{ $receta->paciente->nombre_completo }}
                            </a>
                        </div>
                        <div style="font-size:.75rem;color:var(--texto-secundario);">
                            {{ $receta->paciente->numero_historia }}</div>
                    </div>
                    <div>
                        <div
                            style="font-size:.72rem;color:var(--texto-secundario);text-transform:uppercase;letter-spacing:.04em;">
                ...
                @if ($receta->diagnostico)
                    <div style="margin-bottom:1rem;padding-bottom:1rem;border-bottom:1px solid var(--fondo-borde);">
                        <div
                            style="font-size:.72rem;color:var(--texto-secundario);text-transform:uppercase;letter-spacing:.04em;margin-bottom:.3rem;">
                            Diagnóstico</div>
                ...
                @if ($receta->evolucion)
                    <div style="font-size:.78rem;color:var(--texto-secundario);">
                        ...
                        <a href="{{ route('evoluciones.show', $receta->evolucion) }}"
                            style="color:var(--color-principal);text-decoration:none;">
                            {{ $receta->evolucion->fecha->format('d/m/Y') }} —
                            {{ Str::limit($receta->evolucion->procedimiento, 40) }}
                        </a>
                    </div>
                @endif
            </div>
            ...
            <div class="card-sistema" style="margin-bottom:1rem;">
                <h5
                    style="font-size:.85rem;font-weight:700;text-transform:uppercase;color:var(--color-principal);letter-spacing:.06em;margin-bottom:1rem;padding-bottom:.75rem;border-bottom:2px solid var(--fondo-borde);">
                    <i class="bi bi-capsule me-2"></i>Medicamentos ({{ count($receta->medicamentos ?? []) }})
                </h5>
                @forelse($receta->medicamentos ?? [] as $i => $med)
                    <div
                        style="border:1px solid var(--fondo-borde);border-radius:10px;padding:1rem;margin-bottom:.75rem;background:var(--fondo-app);">
                        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:.5rem;">
                            <div style="flex:1;">
                                <div
                                    style="font-size:.95rem;font-weight:700;color:var(--texto-principal);margin-bottom:.5rem;">
                                    {{ $i + 1 }}. {{ $med['nombre'] ?? '—' }}
                                    @if (!empty($med['presentacion']))
                                        <span
                                            style="font-size:.75rem;font-weight:400;color:var(--texto-secundario);">({{ $med['presentacion'] }})</span>
                                    @endif
                                </div>
                                <div style="display:flex;flex-wrap:wrap;gap:.5rem;font-size:.8rem;">
                                    @if (!empty($med['dosis']))
                                        <span
                                            style="background:var(--color-muy-claro);color:var(--color-principal);padding:.2rem .55rem;border-radius:20px;font-weight:500;">
                                            <i class="bi bi-droplet me-1"></i>{{ $med['dosis'] }}
                                        </span>
                                    @endif
                                    @if (!empty($med['frecuencia']))
                                        <span
                                            style="background:#f0f9ff;color:#0369a1;padding:.2rem .55rem;border-radius:20px;font-weight:500;">
                                            <i class="bi bi-clock me-1"></i>{{ $med['frecuencia'] }}
                                        </span>
                                    @endif
                                    @if (!empty($med['duracion']))
                                        <span
                                            style="background:#fef3c7;color:#92400e;padding:.2rem .55rem;border-radius:20px;font-weight:500;">
                                            <i class="bi bi-calendar me-1"></i>{{ $med['duracion'] }}
                                        </span>
                                    @endif
                                    @if (!empty($med['cantidad']))
                                        <span
                                            style="background:#f3f4f6;color:#374151;padding:.2rem .55rem;border-radius:20px;font-weight:500;">
                ...
                @empty
                    <p style="color:var(--texto-secundario);font-size:.84rem;text-align:center;padding:1rem 0;">Sin
                        medicamentos</p>
                @endforelse
            </div>
            ...
            @if ($receta->indicaciones_generales)
                <div class="card-sistema">
                    <h5
                        style="font-size:.85rem;font-weight:700;text-transform:uppercase;color:var(--color-principal);letter-spacing:.06em;margin-bottom:.75rem;">
                        <i class="bi bi-card-text me-2"></i>Indicaciones generales
                    </h5>
                    <p style="font-size:.9rem;color:var(--texto-principal);white-space:pre-wrap;margin:0;">
                        {{ $receta->indicaciones_generales }}</p>
                </div>
            @endif
        </div>
        ...
        <div class="col-md-4">
            <div class="card-sistema" style="margin-bottom:1rem;">
                <h6
                    style="font-size:.78rem;font-weight:700;text-transform:uppercase;color:var(--texto-secundario);letter-spacing:.04em;margin-bottom:.75rem;">
                    Acciones</h6>
            ...
            <div class="card-sistema">
                <h6
                    style="font-size:.78rem;font-weight:700;text-transform:uppercase;color:var(--texto-secundario);letter-spacing:.04em;margin-bottom:.75rem;">
                    Datos del paciente</h6>
                <div style="font-size:.83rem;">
                    <div style="margin-bottom:.3rem;"><span style="color:var(--texto-secundario);">Nombre:</span>
                        {{ $receta->paciente->nombre_completo }}</div>
                    <div style="margin-bottom:.3rem;"><span style="color:var(--texto-secundario);">Documento:</span>
                        {{ $receta->paciente->tipo_documento }} {{ $receta->paciente->numero_documento }}</div>
                    <div style="margin-bottom:.3rem;"><span style="color:var(--texto-secundario);">Edad:</span>
                        {{ $receta->paciente->edad }} años</div>
                    <div><span style="color:var(--texto-secundario);">Teléfono:</span> {{ $receta->paciente->telefono }}
                    </div>
                </div>
            </div>
        </div>
