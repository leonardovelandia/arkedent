...
<style>
    ...
    .form-textarea:focus { border-color:var(--color-principal); }
...
<div style="display:flex;align-items:center;gap:1rem;margin-bottom:1.5rem;flex-wrap:wrap;">
    ...
    <div>
        <h4 style="font-family:var(--fuente-titulos);font-weight:700;color:#1c2b22;margin:0;">Nuevo Presupuesto</h4>
        <p style="font-size:.82rem;color:#9ca3af;margin:0;">Complete los datos del presupuesto de tratamiento</p>
    </div>
</div>
...
<div class="form-card">
    ...
    <div style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr;gap:1rem;">
        ...
        <div class="form-group">
            <label class="form-label">Fecha de generación <span style="color:#dc2626;">*</span></label>
            ...
            @error('fecha_generacion')<div class="form-error">{{ $message }}</div>@enderror
        </div>
        <div class="form-group">
            <label class="form-label">Validez (días)</label>
            <input type="number" name="validez_dias" id="validez-dias" class="form-input"
                   value="{{ old('validez_dias', 30) }}" min="1" max="365"
                   oninput="calcularVencimiento()">
            @error('validez_dias')<div class="form-error">{{ $message }}</div>@enderror
        </div>
        <div class="form-group">
...
{{-- SECCIÓN 2: Items --}}
<div class="form-card">
    <div class="seccion-titulo"><i class="bi bi-list-ul"></i> Procedimientos del Presupuesto</div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;align-items:start;">
        <div>
            <div class="form-group">
                ...
                @error('descuento_porcentaje')<div class="form-error">{{ $message }}</div>@enderror
            </div>
        </div>
        <div class="totales-card">
            <div class="total-fila">
                <span style="color:#6b7280;">Subtotal:</span>
                <span id="display-subtotal" style="font-weight:600;color:#1c2b22;">$ 0</span>
            </div>
            <div class="total-fila">
                <span style="color:#6b7280;">Descuento:</span>
                <span id="display-descuento" style="font-weight:600;color:#dc2626;">- $ 0</span>
            </div>
...
<div class="form-card">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
        <div class="form-group">
            ...
            <textarea name="condiciones_pago" class="form-textarea" rows="3"
                      placeholder="Ej: 50% al iniciar, 50% al finalizar…">{{ old('condiciones_pago') }}</textarea>
        </div>
        <div class="form-group">
...
<div style="display:flex;gap:.5rem;flex-wrap:wrap;">
    <button type="submit" name="accion" value="borrador" class="btn-morado">
        <i class="bi bi-floppy"></i> Guardar como borrador
    </button>
    <button type="submit" name="accion" value="enviar"
            style="background:linear-gradient(135deg,#0D6EFD,#0a58ca);color:#fff;border:none;border-radius:8px;padding:.5rem 1.25rem;font-size:.875rem;font-weight:500;display:inline-flex;align-items:center;gap:.4rem;cursor:pointer;">
        <i class="bi bi-send"></i> Guardar y enviar al paciente
    </button>
    <a href="{{ route('presupuestos.index') }}" class="btn-gris">
...
function agregarFila(data) {
    ...
    tr.innerHTML =
        ...
        '<td><input type="text" name="items[' + idx + '][procedimiento]" class="inp-item" list="lista-procedimientos" placeholder="Procedimiento…" required value="' + (data.procedimiento || '') + '"></td>' +
        '<td><input type="text" name="items[' + idx + '][diente]" class="inp-item" placeholder="Ej: 11" maxlength="20" value="' + (data.diente || '') + '"></td>' +
...
// Escuchar evento buscador-paciente
