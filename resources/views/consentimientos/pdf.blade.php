...
.hdr { display: table; width: 100%; border-bottom: 2.5px solid {{ $C }}; padding-bottom: 12px; margin-bottom: 16px; }
...
.doc-titulo { text-align: center; padding: 10px 0 14px; }
.doc-tipo { font-size: 13px; font-weight: bold; color: #111827; text-transform: uppercase; letter-spacing: .08em; }
.doc-sub { font-size: 9px; color: #4b5563; margin-top: 3px; }
.info-paciente { display: table; width: 100%; background: #eff6ff; border-left: 3px solid {{ $C }}; padding: 10px 14px; margin-bottom: 18px; }
.info-col { display: table-cell; width: 50%; vertical-align: top; }
.info-lbl { font-size: 7px; font-weight: bold; color: {{ $C }}; text-transform: uppercase; letter-spacing: .5px; }
.info-val { font-size: 10px; color: #111827; font-weight: bold; }
.info-det { font-size: 8px; color: #4b5563; margin-top: 1px; }
.contenido-titulo { font-size: 8px; font-weight: bold; color: {{ $C }}; text-transform: uppercase; letter-spacing: .07em; border-bottom: 1.5px solid {{ $C }}; padding-bottom: 3px; margin-bottom: 12px; }
.contenido-texto { font-size: 10px; line-height: 1.8; color: #111827; white-space: pre-wrap; word-break: break-word; }
.firma-wrap { margin-top: 28px; }
.firma-tabla { display: table; width: 100%; border-top: 1.5px solid {{ $C }}; padding-top: 14px; }
...
.firma-titulo { font-size: 8px; font-weight: bold; color: {{ $C }}; text-transform: uppercase; letter-spacing: .07em; margin-bottom: 6px; }
.firma-img { max-height: 70px; max-width: 220px; display: block; }
...
.sello { display: inline-block; border: 1.5px solid #166534; color: #166534; font-weight: bold; font-size: 8px; padding: 2px 8px; text-transform: uppercase; letter-spacing: 1px; margin-top: 5px; }
...
<div class="info-paciente">
    <div class="info-col">
        ...
        <div class="info-det">{{ $consentimiento->paciente->tipo_documento }} {{ $consentimiento->paciente->numero_documento }}</div>
    </div>
        ...
        <div class="firma-col">
            ...
            @if($consentimiento->firmado)
                ...
                <div class="firma-linea-img">
                    ...
                    @if($consentimiento->fecha_firma)Firmado el {{ $consentimiento->fecha_firma->format('d/m/Y H:i') }} hrs @endif
                    @if($consentimiento->ip_firma) · IP: {{ $consentimiento->ip_firma }}@endif
                </div>
...
<div class="footer">
    ...
    <div class="fl-r">
        {{ $consentimiento->numero_consentimiento ?? ('Documento N° '.$consentimiento->id) }} · Generado el {{ $consentimiento->fecha_generacion->format('d/m/Y') }}
    </div>
</div>
