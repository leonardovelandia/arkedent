...
<style>
    ...
    .estado-chip { display:inline-flex; align-items:center; gap:.4rem; padding:.3rem .75rem; border-radius:20px; font-size:.8rem; font-weight:700; margin-bottom:.5rem; }
</style>
@endpush
...
<div class="d-flex align-items-center gap-2 mb-3">
    <a href="{{ route('citas.show', $cita) }}" style="color:var(--color-principal);font-size:1.1rem;"><i class="bi bi-arrow-left-circle-fill"></i></a>
    <div>
        <h4 style="font-family:var(--fuente-titulos);font-weight:700;color:#1c2b22;margin:0;">Editar Cita</h4>
        <p style="font-size:.82rem;color:#9ca3af;margin:0;">{{ $cita->paciente->nombre_completo }} — {{ $cita->fecha->translatedFormat('d \d\e M \d\e Y') }}</p>
    </div>
</div>

<div class="form-card">
    <h5><i class="bi bi-pencil-square" style="color:var(--color-principal);"></i> Modificar datos de la cita</h5>
    ...
    <div class="campo-wrap">
        ...
        <x-buscador-paciente
            :pacientes="$pacientes"
            :valor-inicial="old('paciente_id', $cita->paciente_id)"
    ...
    <div class="form-row">
        ...
        <div class="campo-wrap">
            ...
            <input type="text" name="procedimiento" class="campo-ctrl {{ $errors->has('procedimiento') ? 'is-invalid' : '' }}"
                   value="{{ old('procedimiento', $cita->procedimiento) }}">
        ...
        <div class="campo-wrap">
            ...
            <div class="timepicker-wrap">
                ...
                <input type="text" id="hora_inicio" name="hora_inicio" placeholder="HH:MM"
                       ...
                       value="{{ old('hora_inicio', substr($cita->hora_inicio ?? '', 0, 5)) }}" autocomplete="off" readonly>
            </div>
        ...
        <div class="campo-wrap">
            ...
            <div class="timepicker-wrap">
                ...
                <input type="text" id="hora_fin" name="hora_fin" placeholder="HH:MM"
                       ...
                       value="{{ old('hora_fin', substr($cita->hora_fin ?? '', 0, 5)) }}" autocomplete="off" readonly>
            </div>
    ...
    <div class="campo-wrap">
        ...
        <select name="estado" class="campo-ctrl {{ $errors->has('estado') ? 'is-invalid' : '' }}">
            ...
            <option value="{{ $val }}" {{ old('estado', $cita->estado) === $val ? 'selected' : '' }}>{{ $lbl }}</option>
            @endforeach
        </select>
    ...
    <div class="campo-wrap">
        ...
        <textarea name="notas" rows="3" class="campo-ctrl {{ $errors->has('notas') ? 'is-invalid' : '' }}"
                  placeholder="Observaciones, indicaciones especiales…">{{ old('notas', $cita->notas) }}</textarea>
