...
<div class="d-flex align-items-center gap-2 mb-3">
    ...
    <h4 style="font-family:var(--fuente-titulos);font-weight:700;color:#1c2b22;margin:0;">Nuevo Tratamiento</h4>
</div>
...
<div class="form-card">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
        <div class="form-group">
            ...
            <select name="paciente_id" id="sel-paciente" class="form-select" required onchange="cargarHistorias(this.value)">
                <option value="">Seleccionar paciente…</option>
                @foreach($pacientes as $p)
                <option value="{{ $p->id }}"
                    {{ old('paciente_id', $pacienteSeleccionado?->id) == $p->id ? 'selected' : '' }}>
                    {{ $p->apellido }}, {{ $p->nombre }}
                </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <select name="historia_clinica_id" id="sel-historia" class="form-select">
                <option value="">Sin historia específica</option>
                @foreach($historias as $h)
                <option value="{{ $h->id }}" {{ old('historia_clinica_id') == $h->id ? 'selected' : '' }}>
...
<div class="form-card">
    ...
    <div class="form-group">
        ...
        <input type="text" name="nombre" class="form-input"
               value="{{ old('nombre') }}" placeholder="Ej: Ortodoncia, Implante, Endodoncia…" required maxlength="255">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:1rem;">
        <div class="form-group">
            ...
            <input type="text" inputmode="numeric" name="valor_total" class="form-input"
                   value="{{ old('valor_total') }}" placeholder="0" required data-money>
    ...
    <div class="form-group">
        ...
        <textarea name="notas" class="form-input" rows="3"
                  placeholder="Notas adicionales sobre el tratamiento…">{{ old('notas') }}</textarea>
...
<div style="display:flex;gap:.5rem;flex-wrap:wrap;">
    <button type="submit" class="btn-morado">
        <i class="bi bi-check-circle"></i> Guardar Tratamiento
    </button>
    <a href="{{ route('tratamientos.index') }}"
...
function cargarHistorias(pacienteId) {
    var sel = document.getElementById('sel-historia');
    sel.innerHTML = '<option value="">Sin historia específica</option>';
    if (!pacienteId) return;
    fetch('/api/pacientes/' + pacienteId + '/historias')
        .then(function(r) { return r.json(); })
        .then(function(data) {
            data.forEach(function(h) {
                var opt = document.createElement('option');
                opt.value = h.id;
                opt.textContent = 'Historia #' + h.id + ' — ' + h.fecha;
                sel.appendChild(opt);
            });
        }).catch(function(){});
}
