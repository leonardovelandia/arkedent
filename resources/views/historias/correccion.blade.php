@extends('layouts.app')
@section('titulo', 'Nota de Corrección — Historia Clínica')
...
<style>
    ...
    .banner-firmado { background:#fffbeb; border:1px solid #fcd34d; border-radius:10px; padding:1rem 1.25rem; margin-bottom:1.5rem; display:flex; gap:.85rem; align-items:flex-start; }
    .banner-firmado-ico { font-size:1.4rem; color:#d97706; flex-shrink:0; margin-top:.05rem; }
    .banner-firmado-titulo { font-weight:700; color:#92400e; font-size:.9rem; margin-bottom:.2rem; }
    .banner-firmado-texto { font-size:.82rem; color:#78350f; line-height:1.5; }
    ...
    .hist-corr-item { border-left:3px solid var(--color-principal); padding:.75rem 1rem; margin-bottom:.75rem; background:var(--fondo-card-alt); border-radius:0 8px 8px 0; }
    .hist-corr-head { display:flex; justify-content:space-between; margin-bottom:.35rem; flex-wrap:wrap; gap:.3rem; }
    .hist-corr-campo { font-size:.8rem; font-weight:600; color:var(--color-principal); }
    .hist-corr-meta { font-size:.75rem; color:#9ca3af; }
    .hist-corr-anterior { font-size:.8rem; color:#9ca3af; text-decoration:line-through; margin-bottom:.25rem; }
    .hist-corr-nuevo { font-size:.8rem; color:#1c2b22; margin-bottom:.25rem; }
    .hist-corr-motivo { font-size:.75rem; color:#6b7280; }
    ...
    body[data-ui="glass"] .banner-firmado-titulo { color:#fde68a !important; }
    body[data-ui="glass"] .banner-firmado-texto { color:rgba(255,255,255,0.75) !important; }
    ...
    body[data-ui="glass"] .hist-corr-item { background:rgba(0,0,0,0.20) !important; border-left-color:rgba(0,234,255,0.70) !important; }
    body[data-ui="glass"] .hist-corr-campo { color:rgba(0,234,255,0.90) !important; }
    body[data-ui="glass"] .hist-corr-meta { color:rgba(255,255,255,0.50) !important; }
    body[data-ui="glass"] .hist-corr-anterior { color:rgba(255,255,255,0.40) !important; }
    body[data-ui="glass"] .hist-corr-nuevo { color:rgba(255,255,255,0.85) !important; }
    body[data-ui="glass"] .hist-corr-motivo { color:rgba(255,255,255,0.60) !important; }
...
{{-- Banner informativo --}}
<div class="banner-firmado">
    <div class="banner-firmado-ico"><i class="bi bi-lock-fill"></i></div>
    <div>
        <div class="banner-firmado-titulo">Historia Clínica Firmada — Solo lectura</div>
        <div class="banner-firmado-texto">
            Esta historia fue firmada el {{ $historia->fecha_firma ? $historia->fecha_firma->format('d/m/Y \a \l\a\s H:i') : '—' }}.
            El contenido original <strong>no puede modificarse</strong>.<br>
            Esta corrección quedará registrada como nota anexa con fecha, hora y responsable.
        </div>
    </div>
</div>
...
<div class="card-sec">
    <div class="card-sec-head"><i class="bi bi-journal-medical" style="color:var(--color-principal);"></i><h6>Documento Original</h6></div>
    <div class="card-sec-body">
        <div class="resumen-dato"><span class="resumen-lbl">Paciente</span><span class="resumen-val">{{ $historia->paciente->nombre_completo }}</span></div>
        <div class="resumen-dato"><span class="resumen-lbl">N° Historia</span><span class="resumen-val">{{ $historia->paciente->numero_historia }}</span></div>
        <div class="resumen-dato"><span class="resumen-lbl">Fecha de firma</span><span class="resumen-val">{{ $historia->fecha_firma ? $historia->fecha_firma->format('d/m/Y H:i') : '—' }}</span></div>
        <div class="resumen-dato"><span class="resumen-lbl">Estado</span><span style="background:#d1fae5;color:#166534;padding:.15rem .55rem;border-radius:6px;font-size:.78rem;font-weight:700;"><i class="bi bi-check-circle-fill"></i> Firmada</span></div>
    </div>
</div>
...
<div class="card-sec">
    ...
    <div class="card-sec-body">
        ...
        <form method="POST" action="{{ route('historias.correccion', $historia) }}">
            @csrf
            ...
            <div style="margin-bottom:1rem;">
                ...
                <select name="campo_corregido" id="campo_corregido" class="form-ctrl" required
                    data-historia="{{ json_encode(collect($camposDisponibles)->keys()->mapWithKeys(fn($k) => [$k => $historia->$k ?? ''])) }}">
            ...
            <div style="margin-bottom:1rem;" id="bloque-valor-actual" style="display:none;">
...
<div class="card-sec">
    <div class="card-sec-head"><i class="bi bi-clock-history" style="color:var(--color-principal);"></i><h6>Historial de Correcciones ({{ $historia->correcciones->count() }})</h6></div>
    <div class="card-sec-body">
        @if($historia->correcciones->isEmpty())
...
(function () {
    ...
    try {
        historiaData = JSON.parse(select.getAttribute('data-historia') || '{}');
    } catch (e) {}
