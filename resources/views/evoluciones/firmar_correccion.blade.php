@extends('layouts.app')
@section('titulo', 'Firmar Corrección — Evolución Clínica')
...
<div style="display:flex;align-items:center;gap:1rem;margin-bottom:1.5rem;flex-wrap:wrap;">
    <a href="{{ route('evoluciones.show', $correccion->evolucion_id) }}"
    ...
    <div>
        <h4 style="font-family:var(--fuente-titulos);font-weight:700;color:#1c2b22;margin:0;">Firmar Corrección — Evolución Clínica
            @if($correccion->numero_correccion)
            <span style="font-family:monospace;font-size:.75rem;font-weight:700;background:#dbeafe;color:#1d4ed8;border-radius:6px;padding:.1rem .5rem;margin-left:.4rem;">{{ $correccion->numero_correccion }}</span>
            @endif
        </h4>
        <p style="font-size:.82rem;color:#9ca3af;margin:0;">
            {{ $correccion->evolucion->paciente->nombre_completo }} · {{ $correccion->evolucion->paciente->numero_historia }}
        </p>
    </div>
</div>
...
<div style="background:#FFF3CD;border:1px solid #FFC107;border-radius:10px;padding:.85rem 1.1rem;display:flex;align-items:center;gap:.75rem;margin-bottom:1.25rem;">
    ...
    <div>
        <div style="font-size:.875rem;font-weight:600;color:#856404;">Corrección a Evolución Clínica</div>
        <div style="font-size:.8rem;color:#856404;margin-top:.15rem;">
            Paciente: <strong>{{ $correccion->evolucion->paciente->nombre_completo }}</strong> —
            Fecha de la evolución: <strong>{{ $correccion->evolucion->fecha_formateada ?? $correccion->evolucion->fecha }}</strong> —
            Procedimiento: <strong>{{ $correccion->evolucion->procedimiento }}</strong><br>
...
<div class="texto-legal">
    ...
    Yo, <strong>{{ $correccion->evolucion->paciente->nombre_completo }}</strong>, declaro que he leído y estoy de acuerdo
    con la corrección realizada a la evolución clínica del
    <strong>{{ $correccion->evolucion->fecha_formateada ?? $correccion->evolucion->fecha }}</strong>.
    Confirmo que la información corregida refleja correctamente lo ocurrido durante mi consulta.
</div>
...
<div class="card-firma">
    ...
    <div style="margin-top:1rem;">
        <a href="{{ route('evoluciones.show', $correccion->evolucion_id) }}" class="btn-gris">
            <i class="bi bi-arrow-left"></i> Volver a la evolución
        </a>
    </div>
</div>
@else
...
(function() {
    ...
    window.confirmarFirma = function() {
        ...
        fetch('{{ route('evoluciones.correccion.firmar', $correccion) }}', {
            method: 'POST',
            headers: {
